            ...
            do g = 1, NGRP
                do i = 1, FE%n_basis
                    ...
                    do j = 1, FE%n_basis
                        ...
                        val = (MAT_DATA(mat_id)%D(g) * (dN_dx(i)*dN_dx(j) + dN_dy(i)*dN_dy(j) + dN_dz(i)*dN_dz(j)) + &
...
subroutine assemble_source_matrices_pcg(MAT_F, MAT_S, FixedSrc, mesh, FE, Quad, mats, n_groups, is_adjoint)
    type(t_mat), allocatable, intent(out) :: MAT_F(:,:), MAT_S(:,:)
    ...
    do ee = 1, mesh%n_elems
        mat_id = mesh%mats(ee)
        do i = 1, FE%n_basis; elem_coords(i, :) = mesh%nodes(mesh%elems(ee, i), :); end do
...
subroutine assemble_multigroup_source_pcg(B, MAT_F, MAT_S, FixedSrc, X_VEC, X_OLD, &
    ...
    type(t_mat), intent(in)           :: MAT_F(:,:), MAT_S(:,:)
...
subroutine calculate_total_production_pcg(total_prod, X_PCG, mesh, FE, Quad, materials, n_groups)
    ...
    do ee = 1, mesh%n_elems
        ...
        do i = 1, FE%n_basis
            elem_coords(i, 1) = mesh%nodes(node_indices(i), 1)
            elem_coords(i, 2) = mesh%nodes(node_indices(i), 2)
            elem_coords(i, 3) = mesh%nodes(node_indices(i), 3)
        end do
        ...
        do q = 1, Quad%NoPoints
            ...
            do g = 1, n_groups
                local_phi = dot_product(X_PCG(g)%vec(node_indices), FE%N(q, :))
    ...
    subroutine ILU0_CSR(AA, JA, IA, L_AA, L_JA, L_IA, U_AA, U_JA, U_IA)
        ...
        L_JA = JA; U_JA = JA
        L_AA = 0.0_dp; U_AA = 0.0_dp

        allocate(w(n), iw(n))
        iw = 0

        do i = 1, n
            ! Scatter row i of A into work vector w
            do p = IA(i), IA(i+1)-1
                w(JA(p)) = AA(p)
                iw(JA(p)) = i
            end do

            ! Update row i based on previous rows. Assumes sorted columns in A.
            do p = IA(i), IA(i+1)-1
                k = JA(p)
                if (k >= i) exit
                
                ! Find U(k,k) from previously computed row k of U
                found_diag = .false.
                do p_diag_k = U_IA(k), U_IA(k+1)-1
                    if (U_JA(p_diag_k) == k) then
                        u_kk = U_AA(p_diag_k)
                        found_diag = .true.
                        exit
                    end if
                ...
                ! Compute L(i,k) = w(k) / U(k,k)
                if (found_diag .and. abs(u_kk) > 1.0d-30) then
                    w(k) = w(k) / u_kk
                else
                    w(k) = 0.0_dp ! Factorization is unstable, treat L(i,k) as zero
                end if

                ! Update remaining elements in row i: w(j) -= L(i,k) * U(k,j)
                if (abs(w(k)) > 1.0d-30) then
                    do p2 = U_IA(k), U_IA(k+1)-1
                        j = U_JA(p2)
                        if (j < k) cycle
                        if (iw(j) == i) then ! If A(i,j) is in the sparsity pattern
                            w(j) = w(j) - w(k) * U_AA(p2)
                        end if
                    end do
                end if
            ...
            ! Gather updated row from w back into L and U CSR structures
            do p = IA(i), IA(i+1)-1
                j = JA(p)
                if (j < i) then
                    L_AA(p) = w(j)
            ...
            ! Reset work vector's sparsity tracker for next row
            do p = IA(i), IA(i+1)-1
                iw(JA(p)) = 0
            end do
        end do
