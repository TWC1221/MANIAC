...
program fem2d_main
    ...
    do outer_iter = 1, max_outer_iter
        ...
        do inner_iter = 1, max_inner_iter
            do i = 1, n_groups
                if (solver_choice == SOLVER_PETSC) then
                    call assemble_multigroup_source_petsc(B_VEC(i), mesh, FE, Quad, materials, X_VEC, X_PRIME_VEC, n_groups, k_eff_prime, i, is_eigenvalue_problem, S_ext)
                    call KSPSolve(ksp_solvers(i), B_VEC(i), X_VEC(i), ierr)
                else
                    call assemble_multigroup_source_pcg(B_PCG(i), mesh, FE, Quad, materials, X_PCG, X_PRIME_PCG, n_groups, k_eff_prime, i, is_eigenvalue_problem, S_ext)
