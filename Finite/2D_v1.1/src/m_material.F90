...
subroutine ParseMaterialDeck(mats, requested_ids, n_groups, filename)
    ...
    do
        ...
        if (ios /= 0 .or. .not. any(requested_ids == id)) cycle
        ...
        do g = 1, n_groups
            if (g == 1) then
                read(line, *) id, mats(id)%SigA(g), mats(id)%Nu(g), &
            ...
            mats(id)%NuSigF(g) = mats(id)%Nu(g) * mats(id)%SigF(g)
        end do
    end do
    close(u)
...
subroutine PrintMaterialSummary(mats, n_groups)
    ...
    integer, intent(in) :: n_groups
    integer :: i, g
    character(len=20) :: fmt_scat

write(fmt_scat, '(A, I1, A)') '(', n_groups, 'ES11.2)'

print*, " "
print*, "=========================================================================== MATERIAL SUMMARY ==========================================================================="
print '(A4,2X,A5,14X,A3,4X,5(A6,6X),3X, A)', "ID", "Name", "Grp", " SigA ", "  Nu  ", " SigF ", "NuSigF", "SigmaR", "Scattering (To G1..Gn)"
print '(A)', "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
...
do i = 1, size(mats)
    if (allocated(mats(i)%SigA)) then
        do g = 1, n_groups
            if (g == 1) then
                print '(I4,3X,A15,1X,I4,1X,5ES12.4,3X,A,' // trim(fmt_scat) // ')', &
                     i, mats(i)%name, g, mats(i)%SigA(g), mats(i)%Nu(g), &
                     mats(i)%SigF(g), mats(i)%NuSigF(g), mats(i)%SigmaR(g), "|", mats(i)%SigmaS(g, :)
            else
                print '(23X,I4,1X,5ES12.4,3X,A,' // trim(fmt_scat) // ')', &
                     g, mats(i)%SigA(g), mats(i)%Nu(g), mats(i)%SigF(g), &
