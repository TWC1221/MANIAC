module m_boundaries 
    ...
    subroutine apply_bcs_csr(val, col_ind, row_ptr, F, n_nodes, bc_left, par_left, bc_right, par_right, geom_m, x1)
        ...
        case (BC_VACUUM)
            call apply_vacuum_csr(val, col_ind, row_ptr, left_node, geom_m, x1)                 ! vacuum strength (e.gg., +0.5 on diag)
        case (BC_ALBEDO)
            call apply_albedo_csr(val, col_ind, row_ptr, F, left_node, par_left)    ! par_left = a in [0,1]
        case (BC_NONE)
        end select
