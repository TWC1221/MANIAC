...
subroutine export_vtk_pcg(filename, FE, mesh, X_PCG, NGRP, refine_level, use_z)
    ...
    integer :: ee, g, ii, jj, kk, a, unit_v, gid, cid, basep, nbasis, node_id
    ...
    allocate(Xp(n_sub_nodes, 3), Up(n_sub_nodes, NGRP), Cells(n_sub_elems, 8), Wp(n_sub_nodes))
    ...
    write(unit_v, '(A)') "SCALARS Weight double 1"
    write(unit_v, '(A)') "LOOKUP_TABLE default"
    do gid = 1, n_sub_nodes
      write(unit_v, '(F20.12)') Wp(gid)
    end do
    ...
    deallocate(xi_grid, eta_grid, zeta_grid, Xp, Up, Cells, Wp)
  ...
  subroutine export_vtk_petsc(filename, FE, mesh, XPETSc, NGRP, refine_level, use_z)
    ...
    allocate(xi_grid(refine_level), eta_grid(refine_level), zeta_grid(refine_level))
    allocate(Xp(n_sub_nodes, 3), Up(n_sub_nodes, NGRP), Cells(n_sub_elems, 8), MatID_p(n_sub_elems), Wp(n_sub_nodes))
    allocate(p_sols(NGRP))

    do g = 1, NGRP
        PetscCall(VecGetArrayRead(XPETSc(g), p_sols(g)%p, ierr))
    end do
    ...
    do g = 1, NGRP
        PetscCall(VecRestoreArrayRead(XPETSc(g), p_sols(g)%p, ierr))
    end do
    deallocate(xi_grid, eta_grid, zeta_grid, Xp, Up, Cells, MatID_p, p_sols, Wp)
