...
contains
    ...
    subroutine read_asmg_mesh(filepath, mesh)
            ...
            if (index(line, '$3D_Patch_Description_Start') > 0) then
                ...
                if (pos > 0) then
                    ...
                    if (iostatus == 0) then
                        ...
                        ! Skip the next lines containing the control point IDs
                        ! We approximate the skip as IDs are usually listed 
                        ! but the next loop iteration will handle non-keywords.
                    end if
                end if
        ...
        do
            ...
            if (index(line, 'POINTS') == 1) then
                ...
                i = i + 1
                call parse_patch_block(unit, mesh%edges(i,:), mesh%edge_mats(i), .true., n_cp=mesh%n_cp_edge(i), &
                ...
                mesh%n_cp_xi_edge(i) = mesh%n_knots_xi_edge(i) - mesh%order - 1
                mesh%n_cp_eta_edge(i) = mesh%n_knots_eta_edge(i) - mesh%order - 1
                ...
                j = j + 1
                call parse_patch_block(unit, mesh%elems(j,:), mesh%mats(j), .false., &
                ...
                mesh%n_cp_zeta(j) = mesh%n_knots_zeta_patch(j) - mesh%order - 1
            end if
        end do
    ...
    subroutine parse_patch_block(u, out_cp, out_id2, is_2d, n_cp, out_knots_xi, out_knots_eta, out_knots_zeta, n_k_xi, n_k_eta, n_k_zeta)
        integer, intent(in) :: u
        ...
        do
            ...
            if (index(l, 'Material_ID:') > 0) then
                ...
                pos = index(l, ':')
                read(l(pos+1:), *) n_val
                read(u, *) (out_knots_xi(m), m=1, n_val)
                if (present(n_k_xi)) n_k_xi = n_val
            else if (index(l, 'KnotVectorEta') > 0 .and. present(out_knots_eta)) then
                pos = index(l, ':')
                read(l(pos+1:), *) n_val
                read(u, *) (out_knots_eta(m), m=1, n_val)
                if (present(n_k_eta)) n_k_eta = n_val
            else if (index(l, 'KnotVectorZeta') > 0 .and. present(out_knots_zeta)) then
                pos = index(l, ':')
                read(l(pos+1:), *) n_val
                read(u, *) (out_knots_zeta(m), m=1, n_val)
                ...
                if (ios == 0 .and. n_val > 0) then
                    ...
                    if (present(n_k_xi)) n_k_xi = n_val
                    if (present(out_knots_eta)) then
                        out_knots_eta(1:n_val) = out_knots_xi(1:n_val)
                        if (present(n_k_eta)) n_k_eta = n_val
                    end if
                    if (present(out_knots_zeta)) then
                        out_knots_zeta(1:n_val) = out_knots_xi(1:n_val)
                        if (present(n_k_zeta)) n_k_zeta = n_val
                    end if
                end if
            end if
        ...
        ! Fallback for 3D elements: if Zeta was not provided, copy Eta
        if (.not. is_2d .and. present(out_knots_zeta)) then
            if (all(out_knots_zeta == 0.0_dp) .and. any(out_knots_eta /= 0.0_dp)) then
                out_knots_zeta = out_knots_eta
                if (present(n_k_zeta) .and. present(n_k_eta)) n_k_zeta = n_k_eta
            end if
        end if
        ! Fallback for 2D/3D patches: if Eta was not provided, copy Xi
        if (present(out_knots_eta)) then
            if (all(out_knots_eta == 0.0_dp) .and. any(out_knots_xi /= 0.0_dp)) then
                out_knots_eta = out_knots_xi
                if (present(n_k_eta) .and. present(n_k_xi)) n_k_eta = n_k_xi
            end if
        end if
