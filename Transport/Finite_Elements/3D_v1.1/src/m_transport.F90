module m_transport
    use m_constants
    use m_types, only: t_mesh, t_finite, t_sn_quadrature, t_material, timer_start, timer_stop
...
contains
    ...
    subroutine Transport_Sweep(mesh, FE, sn_quad, ang_flux, scalar_flux, total_source, sweep_order, n_groups, ref_ID)
        ...
        real(dp) :: b(FE%n_basis), o_n, dir(3), neighbor_flux
        integer :: idx_start, idx_end

        ! Reset scalar flux
        call timer_start('EVAL - Transport Sweep')
        scalar_flux = 0.0_dp
                            ...
                            if (neighbor_elem_id > 0) then 
                                do i_face_node = 1, FE%n_nodes_per_face
                                    ...
                                    b = b - neighbor_flux * (dir(1)*mesh%face_mass_x(:, FE%face_node_map(i_face_node, f), f, ie) + &
                                                             ...
                                                             dir(3)*mesh%face_mass_z(:, FE%face_node_map(i_face_node, f), f, ie))
                                end do
        ...
        !$OMP END PARALLEL DO
        call timer_stop('EVAL - Transport Sweep')
