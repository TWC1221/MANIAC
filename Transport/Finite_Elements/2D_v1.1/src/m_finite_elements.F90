module m_finite_elements
    ...
    subroutine InitialiseFiniteElements(FE, QuadBound, Quad)
        ...
        ! For a tensor-product element with (p+1)x(p+1) nodes, the local indices are:
        ! Face 1 (bottom): nodes 1, 2, ..., p+1
        ! Face 2 (right):  nodes p+1, 2*(p+1), ..., (p+1)*(p+1)
        ! Face 3 (top):    nodes (p+1)*(p+1), ..., p*(p+1)+1  (reversed)
        ! Face 4 (left):   nodes p*(p+1)+1, ..., 1            (reversed)
