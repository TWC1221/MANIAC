...
use m_constants
use m_types, only: t_finite, t_quadrature, t_sn_quadrature, timer_start, timer_stop, int_to_str
implicit none

private
public :: InitialiseQuadrature, LinearQuadrature, SpectralLinearQuadrature, &
          QuadrilateralQuadrature, HexahedralQuadrature, &
          SpectralQuadrilateralQuadrature, SpectralHexahedralQuadrature, &
          AngularQuadrature

contains

subroutine InitialiseQuadrature(FE, Quad2D, Quad3D, QuadSn, is_adjoint, is_SEM)
    type(t_finite), intent(in)              :: FE   
    type(t_quadrature), intent(inout)       :: Quad2D, Quad3D
    ...
    call timer_start('EVAL - Quadrature')

    if (is_SEM) then
        call SpectralLinearQuadrature(Quad1D, FE%order)
        call SpectralQuadrilateralQuadrature(Quad2D, FE%order)
        call SpectralHexahedralQuadrature(Quad3D, FE%order)
    else
        call LinearQuadrature(Quad1D, FE%order + 1)
        call QuadrilateralQuadrature(Quad2D, FE%order + 1)
        call HexahedralQuadrature(Quad3D, FE%order + 1)
    end if

    call AngularQuadrature(QuadSn, is_adjoint)

    call timer_stop('EVAL - Quadrature')
...
subroutine QuadrilateralQuadrature(Quad, IntegOrder)
    ...
    do jj = 1, IntegOrder
        do ii = 1, IntegOrder
            Quad%xi(gp)  = Quad1D%xi(ii)
            Quad%eta(gp) = Quad1D%xi(jj)
            Quad%weights(gp) = Quad1D%weights(ii) * Quad1D%weights(jj)
            gp = gp + 1
        end do
    end do
...
subroutine HexahedralQuadrature(Quad, IntegOrder)
    type(t_Quadrature), intent(out)  :: Quad
    integer, intent(in) :: IntegOrder
    ...
    Quad%n_points = Quad1D%n_points**3
    allocate(Quad%xi(Quad%n_points), Quad%eta(Quad%n_points), Quad%zeta(Quad%n_points), Quad%weights(Quad%n_points))

    gp = 1
    do kk = 1, Quad1D%n_points
        do jj = 1, Quad1D%n_points
            do ii = 1, Quad1D%n_points
                Quad%xi(gp) = Quad1D%xi(ii)
                Quad%eta(gp) = Quad1D%xi(jj)
                Quad%zeta(gp) = Quad1D%xi(kk)
                Quad%weights(gp) = Quad1D%weights(ii) * Quad1D%weights(jj) * Quad1D%weights(kk)
                gp = gp + 1
            end do
        end do
    end do
...
subroutine SpectralLinearQuadrature(Quad, IntegOrder)
...
end subroutine SpectralLinearQuadrature

subroutine SpectralQuadrilateralQuadrature(Quad, IntegOrder)
    type(t_Quadrature), intent(out)   :: Quad
    integer, intent(in)               :: IntegOrder
    ...
    call SpectralLinearQuadrature(Quad1D, IntegOrder)

    Quad%n_points = Quad1D%n_points**2
    allocate(Quad%xi(Quad%n_points), Quad%eta(Quad%n_points), Quad%weights(Quad%n_points))
...
end subroutine SpectralQuadrilateralQuadrature

subroutine SpectralHexahedralQuadrature(Quad, IntegOrder)
    type(t_Quadrature), intent(out)  :: Quad
    integer, intent(in) :: IntegOrder
...
end subroutine SpectralHexahedralQuadrature

subroutine AngularQuadrature(QuadSn, Adjoint)
    type(t_sn_quadrature), intent(inout) :: QuadSn
    ...
    n_levels = QuadSn%order / 2
    n_octant = (n_levels * (n_levels + 1)) / 2
    QuadSn%n_angles = 8 * n_octant !2D ==> 4 * n_octant
    ...
    write(*,'(A,F10.8,A,F10.8)') " >>> S" // trim(int_to_str(QuadSn%order)) // " Verification: M0=", m0, " M2=", m2*3.0_dp
