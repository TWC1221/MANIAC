module m_material
  ...
  subroutine InitialiseMaterials(mats, mesh, filename, printout)
      ...
      integer :: i, max_id, n_unique, j
      logical :: is_requested

      max_id = 0
      if (allocated(mesh%material_ids)) max_id = maxval(mesh%material_ids)
      if (allocated(mesh%surfaces)) then
          do i = 1, size(mesh%surfaces)
              max_id = max(max_id, mesh%surfaces(i)%bc_id)
          end do
      end if
      if (max_id == 0) return
      ...
      do i = 1, max_id
          ...
          if (allocated(mesh%material_ids)) then
              if (any(mesh%material_ids == i)) is_requested = .true.
          end if
          if (.not. is_requested .and. allocated(mesh%surfaces)) then
              do j = 1, size(mesh%surfaces)
                  if (mesh%surfaces(j)%bc_id == i) then
                      is_requested = .true.
                      exit
                  end if
              end do
