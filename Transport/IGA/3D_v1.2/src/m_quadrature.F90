...
subroutine InitialiseQuadrature(FE, mesh, QuadFace, QuadVol, QuadSn, is_adjoint)
    type(t_quadrature), intent(inout)       :: QuadFace, QuadVol
    ...
    n_points_1d = 2 * FE%order + 1
    
    call QuadrilateralQuadrature(QuadFace, n_points_1d)
    call HexahedralQuadrature(QuadVol, n_points_1d)
    call AngularQuadrature(QuadSn%order, QuadSn, is_adjoint)
...
subroutine AngularQuadrature(SN, QuadSn, Adjoint)
    integer, intent(in)            :: SN
    type(t_sn_quadrature), intent(inout) :: QuadSn
    logical, intent(in)            :: Adjoint
    
    integer  :: i, j, k, m, n_levels, n_octant, q, i_sign, dim
    integer  :: ids(3)
...
subroutine HexahedralQuadrature(Quad, IntegOrder)
    ...
    Quad%n_points = IntegOrder**3
    allocate(Quad%xi(Quad%n_points), Quad%eta(Quad%n_points), &
             Quad%zeta(Quad%n_points), Quad%weights(Quad%n_points))
