module m_transport
    ...
    subroutine Transport_Sweep(mesh, FE, sn_quad, ang_flux, scalar_flux, total_source, sweep_order, ref_ID)
        ...
        real(dp) :: b(FE%n_basis), o_n, dir(3)
        ...
        ! We treat the angular_flux passed in as the 'old' values for reflections
        ! to maintain thread safety across angle sweeps.
                ...
                do g = 1, mesh%n_groups
                    ...
                    do f = 1, mesh%n_faces_per_elem
                        if (is_inflow(f)) then
                            ...
                            face_term = dir(1)*mesh%face_mass_x(:,:,f,ie) + &
                                        dir(2)*mesh%face_mass_y(:,:,f,ie) + &
                                        dir(3)*mesh%face_mass_z(:,:,f,ie)
                            ...
                            if (neighbor_elem_id > 0) then 
                                do i_face_node = 1, FE%n_nodes_per_face
                                    ...
                                    ! Internal neighbor flux (already calculated in this sweep due to sweep_order)
                                ...
                                do i_face_node = 1, FE%n_nodes_per_face
                                    ...
                                    ! Reflective flux: Technically lagged by one iteration for stability 
                                    ! in a parallel angle sweep.
                    ...
                    if (any(b /= b)) then
                        write(*,*) "FATAL: NaN detected in sweep for Elem:", ie, " Angle:", mm
                        stop
                    end if
    ...
    subroutine Source_DGFEM(total_src, scalar_flux, k_eff, materials, mesh, FE, n_groups, is_adjoint, is_eigenvalue)
        ...
        do ee = 1, mesh%n_elems
            ...
            if (any(total_src(idx_start:idx_end, :) /= total_src(idx_start:idx_end, :))) then
                 write(*,*) "FATAL: NaN detected in source for Elem:", ee
                 stop
            end if
        end do
        !$OMP END PARALLEL DO
